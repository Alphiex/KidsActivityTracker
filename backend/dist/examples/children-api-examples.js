...
exports.getChildrenWithStats = getChildrenWithStats;
exports.linkActivitiesToChild = linkActivitiesToChild;
...
exports.updateChildInterests = updateChildInterests;
const axios_1 = __importDefault(require("axios"));
const API_BASE_URL = 'http://localhost:3000/api';
let authToken = '';
const apiRequest = axios_1.default.create({
...
async function linkActivitiesToChild(childId) {
    try {
        ...
        if (activities.length > 0) {
            const activityIds = activities.slice(0, 3).map((a) => a.id);
...
async function getRecommendations(childId) {
    try {
...
async function getCalendarData(childIds) {
    try {
        const params = {
            view: 'month',
...
async function completeActivity(childId, activityId) {
    try {
...
async function getActivityHistory(childId) {
    try {
        const params = {
            status: 'completed',
            minRating: 4,
...
async function updateChildInterests(childId) {
    try {
...
//# sourceMappingURL=children-api-examples.js.map