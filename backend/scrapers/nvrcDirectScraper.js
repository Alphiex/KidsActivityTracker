      ...
      const pageActivities = await page.evaluate((section, actType) => {
        ...
        groupItems.forEach((itemRow, idx) => {
          try {
            ...
            // Skip if it doesn't look like an activity session
            ...
            // Find which activity group this belongs to
            let activityName = actType;
            let prevTitle = itemRow.previousElementSibling;
            while (prevTitle) {
              if (prevTitle.classList.contains('bm-group-title-row')) {
                activityName = prevTitle.textContent?.trim() || actType;
                break;
              }
            ...
            const activity = {
              ...
              name: activityName,
                ...
                registrationUrl: (() => {
                  const signUpLink = itemRow.querySelector('a[href*="BookMe4"], a[href*="courseId"]');
