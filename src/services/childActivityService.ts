...
class ChildActivityService {
  ...
  async linkActivity(input: LinkActivityInput): Promise<ChildActivity> {
    try {
      const response = await apiClient.post('/child-activities/link', input);
  ...
  async getChildActivities(childId: string): Promise<ChildActivity[]> {
    try {
      const response = await apiClient.get(`/child-activities/${childId}/activities`);
  ...
  async getActivityChildren(activityId: string): Promise<string[]> {
    try {
      ...
      const response = await apiClient.get('/child-activities/history');
  ...
  async getActivityHistory(filters: ActivityHistoryFilters = {}): Promise<ChildActivity[]> {
    try {
      ...
      const response = await apiClient.get(`/child-activities/history?${params.toString()}`);
      return response.data.activities;
    } catch (error) {
  ...
  async getRecommendedActivities(childId: string): Promise<Activity[]> {
    try {
      const response = await apiClient.get(`/child-activities/${childId}/recommendations`);
      return response.data.activities;
    } catch (error) {
  ...
  async getChildFavorites(childId: string): Promise<ChildActivity[]> {
    try {
      const response = await apiClient.get(`/child-activities/${childId}/favorites`);
  ...
  async getCalendarData(
    ...
    try {
      ...
      const response = await apiClient.get(`/child-activities/calendar?${params.toString()}`);
  ...
  async getActivityStats(childIds?: string[]): Promise<any> {
    try {
      ...
      const response = await apiClient.get(`/child-activities/stats${params}`);
  ...
  async getUpcomingActivities(days = 7): Promise<ChildActivity[]> {
    try {
      const response = await apiClient.get(`/child-activities/upcoming?days=${days}`);
      return response.data.activities;
    } catch (error) {
