'use client';

import Image from 'next/image';

interface ActivityCategory {
  name: string;
  image: string;
  color: string;
  bgColor: string;
  activities: string;
}

const categories: ActivityCategory[] = [
  {
    name: 'Swimming',
    image: '/images/activities/swimming.jpg',
    color: 'text-sky-600',
    bgColor: 'bg-sky-100',
    activities: '150+',
  },
  {
    name: 'Sports',
    image: '/images/activities/sports.jpg',
    color: 'text-green-600',
    bgColor: 'bg-green-100',
    activities: '200+',
  },
  {
    name: 'Arts & Crafts',
    image: '/images/activities/arts_crafts.jpg',
    color: 'text-orange-600',
    bgColor: 'bg-orange-100',
    activities: '120+',
  },
  {
    name: 'Music',
    image: '/images/activities/music.jpg',
    color: 'text-yellow-600',
    bgColor: 'bg-yellow-100',
    activities: '80+',
  },
  {
    name: 'Dance',
    image: '/images/activities/dance.jpg',
    color: 'text-pink-600',
    bgColor: 'bg-pink-100',
    activities: '100+',
  },
  {
    name: 'STEM',
    image: '/images/activities/stem.jpg',
    color: 'text-indigo-600',
    bgColor: 'bg-indigo-100',
    activities: '60+',
  },
  {
    name: 'Nature',
    image: '/images/activities/nature.jpg',
    color: 'text-emerald-600',
    bgColor: 'bg-emerald-100',
    activities: '90+',
  },
  {
    name: 'Camps',
    image: '/images/activities/camps.jpg',
    color: 'text-amber-600',
    bgColor: 'bg-amber-100',
    activities: '150+',
  },
];

export default function ActivityShowcase() {
  return (
    <section className="py-20 bg-gradient-to-b from-white to-[#FFF5F8]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">
            Explore Activities for{' '}
            <span className="gradient-text">Every Interest</span>
          </h2>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            From swimming to STEM, dance to outdoor adventures - find the perfect
            activities to keep your kids engaged and growing.
          </p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          {categories.map((category, index) => (
            <div
              key={index}
              className="group relative overflow-hidden rounded-2xl bg-white shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer"
            >
              {/* Image */}
              <div className="relative h-40 sm:h-48 overflow-hidden">
                <Image
                  src={category.image}
                  alt={category.name}
                  fill
                  className="object-cover group-hover:scale-110 transition-transform duration-500"
                />
                {/* Gradient overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
              </div>

              {/* Content overlay */}
              <div className="absolute bottom-0 left-0 right-0 p-4">
                <h3 className="text-white font-bold text-lg mb-1">
                  {category.name}
                </h3>
                <div className={`inline-flex items-center px-2 py-1 rounded-full ${category.bgColor}`}>
                  <span className={`text-sm font-semibold ${category.color}`}>
                    {category.activities} activities
                  </span>
                </div>
              </div>

              {/* Hover effect - colored border */}
              <div className="absolute inset-0 border-4 border-transparent group-hover:border-white/50 rounded-2xl transition-colors duration-300" />
            </div>
          ))}
        </div>

        {/* CTA */}
        <div className="text-center mt-12">
          <p className="text-gray-600 mb-4">
            Download the app to explore all activities in your city
          </p>
          <div className="flex justify-center gap-4">
            <a
              href="#"
              className="inline-flex items-center px-6 py-3 bg-gray-900 text-white rounded-xl font-medium hover:bg-gray-800 transition-colors"
            >
              <svg className="w-6 h-6 mr-2" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
              </svg>
              App Store
            </a>
            <a
              href="#"
              className="inline-flex items-center px-6 py-3 bg-gray-900 text-white rounded-xl font-medium hover:bg-gray-800 transition-colors"
            >
              <svg className="w-6 h-6 mr-2" viewBox="0 0 24 24" fill="currentColor">
                <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.9 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z"/>
              </svg>
              Play Store
            </a>
          </div>
        </div>
      </div>
    </section>
  );
}
