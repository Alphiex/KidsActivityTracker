...
export default function DownloadButtons({ className = '' }: DownloadButtonsProps) {
  return (
    <div id="download" className={`flex flex-col sm:flex-row gap-3 ${className}`}>
      {/* App Store Button */}
      <a
        href={APP_STORE_URL}
        target="_blank"
      ...
      <div
        className="inline-flex items-center justify-center gap-3 bg-gray-700 text-white px-6 py-3 rounded-xl cursor-not-allowed opacity-75"
      >
        ...
        <div className="text-left">
          ...
          <div className="text-lg font-semibold -mt-1">Coming Soon</div>
        </div>
      </div>
    </div>
  );
