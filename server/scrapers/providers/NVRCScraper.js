  ...
  async extractActivitiesFromPerfectMindPage(page, sectionName, activityType) {
    ...
    const pageActivities = await page.evaluate((section, actType) => {
      ...
      groupItems.forEach((itemRow, idx) => {
        try {
          ...
          const activity = {
            ...
            daysOfWeek: extractNVRCDaysOfWeek(itemText),
            time: itemText.match(/(\d{1,2}:\d{2}\s*[ap]\.?m\.?\s*-\s*\d{1,2}:\d{2}\s*[ap]\.?m\.?)/i)?.[1],
            ageRange: extractNVRCAgeRange(itemText),
            location: extractNVRCLocation(itemText),
            price: parseFloat(itemText.match(/\$([0-9,]+(?:\.\d{2})?)/)?.[1]?.replace(',', '') || '0'),
            availability: extractNVRCAvailability(itemText, itemRow),
            spotsAvailable: extractNVRCSpots(itemText),
            registrationUrl: extractNVRCRegistrationUrl(itemRow),
      ...
      function extractNVRCAgeRange(itemText) {
        ...
        const altMatch = itemText.match(/(\d+)\s*[-–to]+\s*(\d+)\s*(?:yrs?|years?)/i);
        if (altMatch) {
          return {
            min: parseInt(altMatch[1]),
            max: parseInt(altMatch[2])
          };
        }
        return null;
      }
      ...
      function extractNVRCLocation(itemText) {
        const locationKeywords = ['Centre', 'Center', 'Park', 'Arena', 'Pool', 'Field', 'Gym', 'Studio', 'Complex', 'Rink'];
