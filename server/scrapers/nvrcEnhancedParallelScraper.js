  ...
  async saveActivitiesToDatabase(providerId) {
    ...
    for (const activity of this.activities) {
      try {
        ...
        // If still no age range, try to extract from ageRestrictions
        ...
        // If still no age range, try to extract from activity name
        if ((ageMin === null || ageMin === undefined) && activity.name) {
          const extractedAge = extractAgeRangeFromText(activity.name);
          if (extractedAge.min !== null) {
            ageMin = extractedAge.min;
            ageMax = extractedAge.max;
          }
        ...
        if ((ageMin === null || ageMin === undefined) && activity.fullDescription) {
          const extractedAge = extractAgeRangeFromText(activity.fullDescription);
          if (extractedAge.min !== null) {
            ageMin = extractedAge.min;
            ageMax = extractedAge.max;
          }
